AOS.init({
	  ...
	  success: function (html) {
		...
		pSingleHolder.append(
		  '<div id="portfolio-single-' +
			id +
			'" class="portfolio-single-inner"><span class="unslate_co--close-portfolio js-close-portfolio d-flex align-items-center"><span class="close-portfolio-label">Retour au Portfolio</span><span class="icon-close2 wrap-icon-close"></span></span>' +
			getHTMLContent +
			"</div>"
		);
  ...
  var fixBottomLinks = function () {
	var $footer = $(".unslate_co--footer");
	var originalFooterZindex = $footer.css("z-index");
	window.onscroll = function (ev) {
	  if (
		window.innerHeight + window.scrollY >=
		document.body.offsetHeight - 50 // Hardcoded
	  ) {
		$footer.css({
		  "z-index": 99
		});
		...
		$footer.css({
		  "z-index": originalFooterZindex // Hardcoded
		});
	  }
	};
  };
